...
// ─────────────────────────────────────────────
// TIER 8: Security Audit
// ─────────────────────────────────────────────
console.log('TIER 8: Security Audit');
console.log('────────────────────────────────────────');
{
  const { spawnSync } = require('child_process');
  const cliDir = __dirname;

  const auditRes = spawnSync('npm', ['audit', '--audit-level=high'], {
    cwd: cliDir,
    encoding: 'utf8',
    shell: true,
  });
  ...
  if (auditRes.status === 0) {
    ...
    console.log('  ❌ npm audit: vulnerabilities found');
    failed++;
  }

  const auditJsonRes = spawnSync('npm', ['audit', '--json'], {
    cwd: cliDir,
    encoding: 'utf8',
    shell: true,
  });
  ...
  try {
    const auditData = JSON.parse(auditJsonRes.stdout || '{}');
    const meta = auditData.metadata?.vulnerabilities || {};
    const total = Object.values(meta).reduce((sum, n) => sum + (typeof n === 'number' ? n : 0), 0);
    const high = meta.high || 0;
    const critical = meta.critical || 0;
    console.log(`  Info: ${total} vulnerabilities (${high} high, ${critical} critical)`);
  } catch (err) {
    console.log('  ⚠️  Could not parse npm audit --json output');
  }
}
