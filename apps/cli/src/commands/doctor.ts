...
function detectFormat(pack: any): PackFormat {
  ...
  // studio-export: tables is an object keyed by name, columns are objects
  if (pack.tables && typeof pack.tables === 'object' && !Array.isArray(pack.tables)) {
    const firstKey = Object.keys(pack.tables)[0];
    if (firstKey && pack.tables[firstKey]?.columns) return 'studio-export';
    if (firstKey && pack.tables[firstKey]?.match) return 'studio-export';
    return 'cli-object';
  }
  ...
  if (Array.isArray(pack.tables)) {
    ...
    // studio-v4: columns is array, items have .id and .fkTarget
...
function buildIdToNameMap(tables: any[]): Map<string, { tableName: string; columns: Map<string, string> }> {
  ...
  for (const table of tables) {
    ...
    if (table.id && table.name) {
      map.set(table.id, { tableName: table.name, columns: colMap });
    }
  }
  return map;
}
...
function convertStudioV4ToExport(pack: any): any {
  ...
  for (const table of pack.tables) {
    ...
    for (const col of table.columns) {
      const colDef: any = {
        strategy: col.strategy || inferStrategy(col.name, col.type),
      };

      // Copy options if present and non-empty
      if (col.options && Object.keys(col.options).length > 0) {
        colDef.options = col.options;
      }

      // Convert enum values/weights from options
      ...
      if (col.isFK && col.fkTarget) {
        ...
        if (targetTable) {
          ...
          colDef.strategy = 'uuid'; // FK columns generate by referencing parent
        }
      }
...
function inferStrategy(name: string, type: string): string {
  ...
  if (t.includes('timestamp') || t.includes('date') || t.includes('time')) return 'past_date';
...
function checkDateStrategies(pack: any, format: PackFormat): Diagnosis {
  ...
  const knownDateStrategies = ['past_date', 'future_date', 'timestamp', 'date', 'recent_date', 'date_of_birth'];
  ...
  if (format === 'studio-v4' && Array.isArray(pack.tables)) {
    for (const table of pack.tables) {
      for (const col of table.columns || []) {
        ...
        if (type.includes('date') || type.includes('time') || type.includes('timestamp')) {
          if (!knownDateStrategies.includes(col.strategy)) {
    ...
    for (const [tableName, tableDef] of Object.entries(pack.tables)) {
      const cols = (tableDef as any).columns || {};
      for (const [colName, colDef] of Object.entries(cols)) {
        const strategy = (colDef as any).strategy || '';
        if (strategy.includes('date') || strategy.includes('time') || strategy.includes('timestamp')) {
          // These are fine — they have date strategies
        } else if (colName.includes('_at') || colName.includes('date') || colName.includes('time')) {
          if (!knownDateStrategies.includes(strategy)) {
  ...
  if (badDates.length === 0) {
    return { level: 'ok', check: 'Date Strategies', message: 'All date/timestamp columns have valid strategies', fixable: false };
  }
  return {
    level: 'warning',
    check: 'Date Strategies',
    message: `${badDates.length} date column(s) have missing or invalid strategies`,
    fixable: true,
    details: badDates,
  };
}
...
function checkOrphanedFKs(pack: any, format: PackFormat): Diagnosis {
  ...
  if (format === 'studio-v4' && Array.isArray(pack.tables)) {
    ...
    for (const table of pack.tables) {
      for (const col of table.columns || []) {
        if (col.isFK && col.fkTarget) {
          if (!idMap.has(col.fkTarget.tableId)) {
    ...
    for (const [tableName, tableDef] of Object.entries(pack.tables)) {
      ...
      for (const [colName, colDef] of Object.entries(cols)) {
        ...
        if (fk && !tableNames.has(fk.table)) {
          orphaned.push(`${tableName}.${colName} → table "${fk.table}" not found`);
        }
      }
    }
  }
  ...
  if (orphaned.length === 0) {
    return { level: 'ok', check: 'FK References', message: 'All foreign keys reference valid tables', fixable: false };
  }
  return {
    level: 'critical',
    check: 'FK References',
    message: `${orphaned.length} orphaned FK reference(s)`,
    fixable: false,
    details: orphaned,
  };
}
...
function checkEmptyEnums(pack: any, format: PackFormat): Diagnosis {
  ...
  if (format === 'studio-v4' && Array.isArray(pack.tables)) {
    for (const table of pack.tables) {
      for (const col of table.columns || []) {
        if (col.strategy === 'enum') {
          const values = col.options?.values || [];
          if (values.length === 0) {
            emptyEnums.push(`${table.name}.${col.name}`);
          }
        }
      }
    }
    ...
    for (const [tableName, tableDef] of Object.entries(pack.tables)) {
      const cols = (tableDef as any).columns || {};
      for (const [colName, colDef] of Object.entries(cols)) {
        if ((colDef as any).strategy === 'enum') {
          const values = (colDef as any).options?.values || [];
          if (values.length === 0) {
            emptyEnums.push(`${tableName}.${colName}`);
          }
        }
      }
    }
  ...
  if (emptyEnums.length === 0) {
    return { level: 'ok', check: 'Enum Values', message: 'All enum columns have values defined', fixable: false };
  }
  return {
    level: 'warning',
    check: 'Enum Values',
    message: `${emptyEnums.length} enum column(s) have empty values`,
    fixable: false,
    details: emptyEnums,
  };
}
...
function checkDuplicateNames(pack: any, format: PackFormat): Diagnosis {
  ...
  if (format === 'studio-v4' && Array.isArray(pack.tables)) {
    ...
    for (const [name, count] of seen) {
      if (count > 1) dupes.push(`Table "${name}" appears ${count} times`);
    }
  } else if (format === 'studio-export' && typeof pack.tables === 'object') {
    // Object keys are unique by definition, but check case collisions
    const keys = Object.keys(pack.tables);
    const seen = new Map<string, string[]>();
    for (const key of keys) {
      const lower = key.toLowerCase();
      if (!seen.has(lower)) seen.set(lower, []);
      seen.get(lower)!.push(key);
    }
    for (const [, names] of seen) {
      if (names.length > 1) dupes.push(`Case collision: ${names.join(', ')}`);
    }
  }

  if (dupes.length === 0) {
    return { level: 'ok', check: 'Duplicate Names', message: 'No duplicate or colliding table names', fixable: false };
  }
  return {
    level: 'warning',
    check: 'Duplicate Names',
    message: `${dupes.length} naming issue(s)`,
    fixable: false,
    details: dupes,
  };
}
...
function checkMissingStrategies(pack: any, format: PackFormat): Diagnosis {
  ...
  if (format === 'studio-v4' && Array.isArray(pack.tables)) {
    for (const table of pack.tables) {
      for (const col of table.columns || []) {
        if (!col.strategy || col.strategy === '' || col.strategy === 'none') {
          missing.push(`${table.name}.${col.name} (type: ${col.type})`);
        }
      }
    }
  } else if (format === 'studio-export' && typeof pack.tables === 'object') {
  ...
  if (missing.length === 0) {
    return { level: 'ok', check: 'Generator Strategies', message: 'All columns have generation strategies', fixable: false };
  }
  return {
    level: 'warning',
    check: 'Generator Strategies',
    message: `${missing.length} column(s) missing generation strategies`,
    fixable: true,
    details: missing,
  };
}
...
function countTablesAndColumns(pack: any, format: PackFormat): { tables: number; columns: number; fks: number } {
  ...
  if (format === 'studio-v4' && Array.isArray(pack.tables)) {
    ...
    for (const table of pack.tables) {
      ...
      fks += (table.columns || []).filter((c: any) => c.isFK).length;
    }
  } else if (format === 'studio-export' && typeof pack.tables === 'object') {
    const entries = Object.entries(pack.tables);
    tables = entries.length;
    for (const [, tableDef] of entries) {
      const cols = Object.entries((tableDef as any).columns || {});
      columns += cols.length;
      fks += cols.filter(([, c]) => (c as any).foreignKey).length;
    }
  }
...
export async function doctorCommand(options: {
  ...
  console.log(`\n\u{1FA7A} RealityDB Doctor \u2014 Pack Diagnosis`);
  ...
  if (format === 'studio-v4') {
    diagnoses.push({
      ...
      message: 'Pack is studio-v4 format \u2014 CLI expects studio-export',
      fixable: true,
      details: [
        'FK references use fkTarget.tableId/columnId (need foreignKey.table/column)',
        'Tables are array-based with .id fields (need object-keyed by name)',
        'Fix: --fix will convert to studio-export format',
      ],
    });
  ...
  for (const d of diagnoses) {
    const icon = d.level === 'ok' ? '\u2705' : d.level === 'warning' ? '\u26A0\uFE0F ' : '\u274C';
    const label = d.level === 'ok' ? 'OK' : d.level === 'warning' ? 'WARN' : 'CRITICAL';
    console.log(`   ${icon} ${label}: ${d.check}`);
    console.log(`      ${d.message}`);
    if (d.details) {
      for (const detail of d.details.slice(0, 5)) {
        console.log(`      \u2022 ${detail}`);
      }
      if (d.details.length > 5) {
        console.log(`      ... and ${d.details.length - 5} more`);
      }
    }
    console.log();
  }
  ...
  if (fixable > 0 && !options.fix) {
    console.log(`   ${fixable} issue(s) are auto-fixable \u2014 run with --fix to repair`);
  }
  ...
  if (options.fix) {
    ...
    // Fix 1: Convert studio-v4 → studio-export
    if (format === 'studio-v4') {
      fixed = convertStudioV4ToExport(pack);
      console.log(`   \u2705 Converted studio-v4 \u2192 studio-export format`);
      fixCount++;
    }

    // Fix 2: Infer missing strategies (on the converted or original pack)
    if (typeof fixed.tables === 'object' && !Array.isArray(fixed.tables)) {
      for (const [tableName, tableDef] of Object.entries(fixed.tables)) {
        const cols = (tableDef as any).columns || {};
        for (const [colName, colDef] of Object.entries(cols)) {
          if (!(colDef as any).strategy) {
            const inferred = inferStrategy(colName, '');
            (colDef as any).strategy = inferred;
            console.log(`   \u2705 Inferred strategy for ${tableName}.${colName}: ${inferred}`);
            fixCount++;
          }
        }
      }
    ...
    if (fixCount === 0) {
      ...
      const outPath = options.output ? path.resolve(options.output) : packPath;
