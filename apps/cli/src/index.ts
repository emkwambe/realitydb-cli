...
async function runHandler(options: any) {
    ...
    if (cardScale !== 1.0 && pack?.relationships?.some((r: any) => r.cardinality)) {
      ...
      // Seed determinism: parse the seed once and derive a deterministic base
      // timestamp (mirrors deriveBaseEpoch() in packages/engine/src/generators.ts).
      const seedValue = options.seed ? parseInt(String(options.seed), 10) : undefined;
      const genAt = (seedValue !== undefined && !Number.isNaN(seedValue))
        ? new Date(new Date('2024-01-01').getTime() + (seedValue % (365 * 24 * 60 * 60 * 1000))).toISOString()
        : new Date().toISOString();

      // Generate data
      const { allData, actualTotal, elapsed } = generateData(ordered, rowsPerTable, pack, seedValue);
    ...
    recordRowUsage(actualTotal); // Track monthly usage
      ...
      if (format === 'sql') {
        ...
        // H9: stream SQL to disk — avoids V8 512MB string limit on large datasets.
        // Streaming SHA-256 keeps content_hash correct without materializing the string.
        const { createHash } = await import('crypto');
        const sqlHash = createHash('sha256');
        const sqlStream = fs.createWriteStream(outputFile, { encoding: 'utf-8' });
        const writeChunk = (s: string) => { sqlStream.write(s); sqlHash.update(s); };
        writeChunk(`-- ============================================\n`);
        writeChunk(`-- Generated by RealityDB CLI v${VERSION}\n`);
        writeChunk(`-- Template: ${templateName}\n`);
        writeChunk(`-- Total rows: ${actualTotal.toLocaleString()}\n`);
        writeChunk(`-- Generated at: ${genAt}\n`);
        writeChunk(`-- ============================================\n\n`);
        if (!options.dataOnly) {
          writeChunk(`-- ============================================\n`);
          writeChunk(`-- SCHEMA\n`);
          ...
          for (const table of ordered) {
            if (options.dropTables) {
              writeChunk(`DROP TABLE IF EXISTS "${table.name}" CASCADE;\n`);
            }
            writeChunk(generateCreateTable(table));
          }
        }
        writeChunk(`-- ============================================\n`);
        writeChunk(`-- DATA\n`);
        ...
        for (const table of ordered) {
          ...
          if (tableData && tableData.length > 0) {
            writeChunk(generateInsertStatements(table.name, tableData));
          }
        }
        // H9: content_hash from streaming SHA-256 — no full string needed
        const streamedContentHash = sqlHash.copy().digest('hex').substring(0, 16);
        // Auto-watermark: embed _realitydb_meta in every SQL output
        const sqlContent = ''; // H9: not materialized — hash computed above
        ...
        try {
          ...
          const claims = buildClaims({
            ...
            sqlContent: streamedContentHash, // H9: streaming hash
          });
          // Seed determinism: pin the embedded watermark timestamp to the seeded epoch.
          if (seedValue !== undefined && !Number.isNaN(seedValue)) claims.generated_at = genAt;
          ...
          if (privKey) {
            ...
            finalSql = watermark; // H9: stream has body
            ...
            finalSql = basicMeta; // H9: stream has body
          }
          console.log(`   \u{1F3F7}\uFE0F  Watermark: _realitydb_meta table embedded`);
        } catch (wmErr) {
          console.log(`   \u26A0\uFE0F  Watermark skipped: ${wmErr.message}`);
        }
        // H9: write watermark tail to stream then close and await flush
        if (finalSql) { sqlStream.write(finalSql); sqlHash.update(finalSql); }
        await new Promise<void>((res, rej) => {
          sqlStream.end((err?: Error | null) => err ? rej(err) : res());
        });
